...
t += '[APK WORKFLOW - MT MCP] IMPORTANT: remote.mt_mcp.mt_apk_open requires a RELATIVE path (file name only, e.g. Shadowrocket.apk), NEVER an absolute path like /storage/emulated/0/...; if mt_apk_open fails with INVALID_ARGUMENT, retry with the file name only. After mt_apk_open succeeds it returns workspaceId - use EXACTLY that id for all later mt_apk_* calls; never call mt_apk_search/mt_apk_list before mt_apk_open succeeds. mt_apk_list_available_apks REQUIRES limit (int, e.g. 50). IMPORTANT: workspaceId MUST be obtained from the REAL output of workspace_info or mt_apk_open_* tools - NEVER invent or guess a workspaceId; if unsure, call workspace_info FIRST to get the real id. ';
 t += '[APK EDITING WORKFLOW] To change text inside the APK follow EXACTLY this order: 1) remote.mt_mcp.mt_apk_edit_open {workspaceId:<real ws id>} -> returns editSessionId (KEEP it, reuse for all later calls; empty editSessionId means base workspace and edits will FAIL with EDIT_SESSION_NOT_FOUND). 2) remote.mt_mcp.mt_apk_read_text {workspaceId, editSessionId, locator:"dex_class:LXxx;"} -> COPY data.targetVersion EXACTLY (e.g. sha256:...). 3) remote.mt_mcp.mt_apk_edit_text accepts TOP-LEVEL keys ONLY: workspaceId, editSessionId, locator, targetVersion, edits. The edits value MUST be a JSON ARRAY of objects {mode, matchText, writeText}. NEVER put mode/matchText/writeText at top level - they are INVALID there and cause "Missing parameters". Example: <median_name>remote.mt_mcp.mt_apk_edit_text</median_name> {"workspaceId":"<real ws id>","editSessionId":"<from edit_open>","locator":"dex_class:LXxx;","targetVersion":"sha256:<from read_text>","edits":[{"mode":"replace_match","matchText":"<old smali line>","writeText":"<new smali line>"}]} 4) verify with remote.mt_mcp.mt_apk_edit_check {runBuildChecks:false}; 5) when all edits done call remote.mt_mcp.mt_apk_build. If edit_text replies TARGET_VERSION_MISMATCH, re-read with read_text and copy the NEW data.targetVersion. ';
