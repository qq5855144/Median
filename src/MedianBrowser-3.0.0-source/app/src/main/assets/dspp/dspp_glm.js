// ==UserScript==
// @name         Median GLM 工具桥接
// @namespace    median.glm-bridge
// @version      1.15.0
// @description  为 chatglm.cn 注入 Median 本地设备工具链（MCP 桥接、工具调用解析、自动续跑、预算接力、流中断自恢复、回复确认看门狗、反循环防护、结果分析-计划-行动约束、APK编辑工作流、参数模板纠偏、重复失败强制纠偏、Python原生工具意图拦截）
// @match        *://chatglm.cn/*
...
(function () {
  'use strict';
  if (window.__medianGlm) return;
  window.__medianGlm = true;
  ...
  var CHAT_URL_MARK = '/chatglm/backend-api/assistant/stream';
  var CHAT_API = '/chatglm/backend-api/assistant/stream';
  ...
  function glmIsChatHost(u) {
    try {
      var h = new URL(String(u), location.href).hostname || '';
      return /(^|\.)(chatglm\.cn)$/i.test(h);
  ...
  (function installNavGuard() {
    function guard(url) {
      try {
        var s = String(url);
        if (/^https?:\/\//i.test(s) && !glmIsChatHost(s) && window.__glmPanelOpen) {
          window.__glmPanelOpen(s);
    ...
    window.__glmPanelCount = 0;
  })();

  // ---------- MCP 连接（8788 固定 + 探测兜底） ----------
  window.__glmMcu = '';
  window.__glmMcuProbing = false;
  window.__glmMcuBase = function () {
    try {
      if (window.__glmMcu) return window.__glmMcu;
      // v1.14.10: 不再同步逐端口探测——同步XHR在端口不可达时会阻塞主线程导致页面卡死。
      // 优先用注入端口, 否则默认8788, 立即返回; 后台异步探测候选端口并更新
      if (window.__mcpPort && window.__mcpPort > 0) window.__glmMcu = 'http://127.0.0.1:' + window.__mcpPort + '/mcp';
      else window.__glmMcu = 'http://127.0.0.1:8788/mcp';
      try {
        if (!window.__glmMcuProbing) {
          window.__glmMcuProbing = true;
          (function () {
            var cand = [];
            ...
            var idx = 0;
            var probe = function () {
              if (idx >= cand.length) { window.__glmMcuProbing = false; return; }
              var p = cand[idx++];
              try {
                ...
                x.onreadystatechange = function () {
                  ...
                  try {
                    ...
                    if (x.status === 200 && d && d.result && d.result.tools) {
                      window.__glmMcu = 'http://127.0.0.1:' + p + '/mcp';
                      window.__glmMcuProbing = false;
                      return;
                    }
                  } catch (e) {}
      ...
      return window.__glmMcu;
    } catch (e) {
      window.__glmMcu = 'http://127.0.0.1:8788/mcp';
      return window.__glmMcu;
    }
  };
  window.__glmRemoteTools = [];
  window.__glmRemoteTs = 0;
  ...
  function fetchRemoteTools() {
    try {
      var now = Date.now();
      if (window.__glmRemoteTools.length > 0 && now - window.__glmRemoteTs < 15000) return window.__glmRemoteTools;
      // v1.14.10: 后台异步刷新——同步XHR在MCP不可达时会无限阻塞主线程导致页面卡死
      if (!window.__glmRemoteLoading) {
        window.__glmRemoteLoading = true;
        try {
          var x = new XMLHttpRequest();
          x.open('POST', window.__glmMcuBase(), true);
          x.timeout = 5000;
          x.setRequestHeader('Content-Type', 'application/json');
          x.onreadystatechange = function () {
            if (x.readyState !== 4) return;
            ...
            try {
              ...
              if (tools.length) { window.__glmRemoteTools = tools; window.__glmRemoteTs = Date.now(); }
            } catch (e) {}
          };
          x.ontimeout = function () { window.__glmRemoteLoading = false; };
          x.onerror = function () { window.__glmRemoteLoading = false; };
          x.send(JSON.stringify({ jsonrpc: '2.0', id: Date.now(), method: 'tools/list', params: {} }));
        } catch (e) { window.__glmRemoteLoading = false; }
      }
      return window.__glmRemoteTools.length ? window.__glmRemoteTools : __glmFallbackTools;
    } catch (e) { return __glmFallbackTools; }
  }
          ...
          if (!a.edits || !a.edits.length) {
            ...
            if (_item) {
              ...
              if (_mt !== '' || _wt !== '') {
                ...
                a.edits = [_item];
                console.log('[GlmBridge] edit_text 自动包装 edits[] (guess fields normalized)');
              }
            }
          }
          if (a.edits && a.edits.length) {
  ...
  window.__glmDoneSigs = []; // [{sig, ts}] 带时间戳，去重仅限短时间窗（跨会话不误伤）
  window.__glmStreaming = false;
  window.__glmStreamTs = 0;
  window.__glmAckPending = false;
  window.__glmAckTs = 0;
  window.__glmLastAssistTs = 0;
  window.__glmLastReq = null;
  window.__glmLastResp = '';
  window.__glmFailCount = 0;
  window.__glmDiag = { injectedAt: Date.now(), reqs: 0, streams: 0, uiSends: 0, protoSends: 0, capReasons: [] };

  function glmIsDupSig(sig) {
    var now = Date.now(), alive = [];
    for (var i = 0; i < window.__glmDoneSigs.length; i++) {
      var it = window.__glmDoneSigs[i];
      if (now - it.ts < 60000) {
        alive.push(it);
        if (it.sig === sig) { window.__glmDoneSigs = alive; return true; }
      }
    }
    window.__glmDoneSigs = alive;
    return false;
  }
  var __glmAntiLoopTs = 0;
  function glmMarkSig(sig) {
    var now = Date.now();
    var list = window.__glmDoneSigs;
    list.push({ sig: sig, ts: now });
    // v1.14.4: 确认循环终结——检测分析类工具反复执行/重复搜索/重复edit_open,
    // 用 glmSendGuide(协议层优先; uiSendText 会被 isTrusted 拦截失效) 引导 AI 直接进入修改流程。
    ...
    if (recent.length >= 8 && names.slice(-8).every(isAnalyze) && !names.some(isEdit) && now - __glmAntiLoopTs > 90000) {
    ...
    if (!guide) {
      ...
      if (dupQ && now - __glmAntiLoopTs > 90000) {
    ...
    if (!guide) {
      ...
      if (eoN >= 2 && now - __glmAntiLoopTs > 90000) {
        guide = '[系统干预]编辑会话已创建。不要重复 mt_apk_edit_open，直接使用已有 editSessionId 调用 mt_apk_edit_text 完成修改，然后 edit_check、build。';
      }
    }
    if (guide) {
      __glmAntiLoopTs = now;
      console.log('[GlmBridge] v1.14.4 loop-guide:', String(guide).slice(0, 120));
      setTimeout(function () {
        try { glmSendGuide(guide); } catch (e) { console.log('[GlmBridge] guide send err', String(e)); }
      }, 1200);
    }
  }
  // 新用户消息到来时清空去重记录（跨会话/跨任务不再误伤相同签名）
  function glmResetDup() { window.__glmDoneSigs = []; }
  ...
  function toolAndContinue(nm, args) {
    ...
    if ((full === 'browser_open' || full === 'browser_nav') && args) {
      ...
      if (bu && /^https?:\/\//i.test(bu) && !glmIsChatHost(bu)) {
        full = 'browser_panel_open';
        args = { url: cleanPanelUrl(bu) };
      }
    }
    window.__glmBusy = true;
    runTool(full, args).then(function (result) {
      window.__glmBusy = false;
      window.__glmPendingResult = JSON.stringify(result);
      ...
      console.log('[GlmBridge] tool done', full, JSON.stringify(result).slice(0, 200));
      var sig = full + '|' + JSON.stringify(args || {});
      glmMarkSig(sig);
      if (result && result.ok === false) window.__glmFailCount++;
      else window.__glmFailCount = 0;
    ...
    try {
      ...
      try { window.__glmDiag.protoSends = (window.__glmDiag.protoSends || 0) + 1; } catch (e) {}
      ...
      (window.__glmFetchWrapper || window.fetch)(last.url, {
        ...
        window.__glmStreaming = false;
        console.log('[GlmBridge] autoContinue err', String(e));
        if (onFail) onFail();
      });
    } catch (e) { console.log('[GlmBridge] sendChatText err', String(e && e.message || e)); }
  }
  ...
  function autoContinue() {
    ...
    try {
      var _pendRaw8 = window.__glmPendingResult;
      if (/resource_exhausted|REASON_CHAT_IN_PROGRESS|CHAT_IN_PROGRESS|unauthenticated|token is expired|invalid user token|budget exhausted|工具调用预算|预算已耗尽|25步.{0,12}(限制|预算)|Do not attempt to search|You have exhausted|超出限制|无法再调用任何工具|TOKEN_LENGTH_TOO_LONG|聊得太长/i.test(_pendRaw8)) {
        window.__glmPendingResult = null;
        window.__glmDiag.silentErrors = (window.__glmDiag.silentErrors||0)+1;
        console.log('[GlmBridge] infra error result silenced:', String(_pendRaw8).slice(0,120));
        return;
      }
    } catch (eSE8) {}
    ...
    var _fixHint = '';
    try {
      var _pend9 = String(window.__glmPendingResult || '');
      ...
      if (_isFail9) window.__glmFailSeq = (window.__glmFailSeq || 0) + 1;
      else window.__glmFailSeq = 0;
      if (window.__glmFailSeq >= 3) {
        _fixHint += '\n[\u5f3a\u5236\u7ea0\u504f] \u540c\u4e00\u5de5\u5177\u5df2\u8fde\u7eed\u5931\u8d25 ' + window.__glmFailSeq + ' \u6b21\uff0c\u505c\u6b62\u8bd5\u9519\u5faa\u73af\uff01\u2460\u56de\u770b\u672c\u4f1a\u8bdd\u9996\u6761\u7cfb\u7edf\u6d88\u606f\u4e2d\u7684\u5de5\u5177\u5217\u8868\u4e0e [APK EDIT] \u5de5\u4f5c\u6d41\u793a\u4f8b\uff1b\u2461\u7528 <median_name> \u6807\u7b7e\u4e00\u6b21\u8c03\u7528\uff0c\u53c2\u6570\u7528\u771f\u5b9e\u503c\uff08edits \u6570\u7ec4\u5143\u7d20\u53ea\u542b mode/matchText/writeText\uff09\uff1b\u2462targetVersion \u62a5\u9519\u5c31\u5148 mt_apk_read_text \u91cd\u65b0\u8bfb\u53d6\u6700\u65b0\u503c\u518d\u7f16\u8f91\uff1b\u2463\u7981\u6b62\u8f93\u51fa\u603b\u7ed3\u3001\u7981\u6b62\u653e\u5f03\u3001\u7981\u6b62\u4f7f\u7528 Python \u4ee3\u7801\u5de5\u5177\u3002\u7acb\u5373\u6267\u884c\uff01\n';
        window.__glmFailSeq = 0;
      }
      if (window.__glmPyWarn > 0) {
    ...
    var fallback = function () {
      if (uiSendText(txt)) { window.__glmUiRetryCount = 0; return; }
      if (window.__glmUiRetryCount < 6) {
        window.__glmUiRetryCount++;
        window.__glmPendingResult = pending;
        console.log('[GlmBridge] both channels failed, retry#' + window.__glmUiRetryCount);
        setTimeout(autoContinue, 2500);
      } else {
        window.__glmUiRetryCount = 0;
  ...
  window.__glmRelayFired = false;
  function glmCollectContext() {
    try {
  ...
  function glmStartRelay(reason) {
    if (window.__glmRelayFired) return;
    window.__glmRelayFired = true;
    var reasonTxt = reason === 'token-limit' ? '对话长度上限' : '工具预算限制';
    var ctx = glmCollectContext();
    var oldPath = location.pathname;
    console.log('[GlmBridge] relay(' + reasonTxt + '): switching to new chat');
    try {
    ...
    } catch (e) { console.log('[GlmBridge] relay click err', String(e)); }
    var tries = 0;
    var iv = setInterval(function () {
      ...
      if (ready) {
        ...
        console.log('[GlmBridge] relay message sent');
      } else if (tries > 90) { clearInterval(iv); console.log('[GlmBridge] relay give up'); }
    }, 1200);
  }
  function glmDetectRelay(buf) {
    if (!buf || window.__glmRelayFired) return false;
    if (/工具调用预算已耗尽|预算已耗尽|25步.{0,12}(限制|预算)|tool call budget exhausted|Do not attempt to search|You have exhausted|超出限制|无法再调用任何工具/i.test(buf)) {
      console.log('[GlmBridge] budget notice detected -> correct first, relay as fallback');
      // v1.10.1：纠正消息（finalize 中已排队发送）先拉回 AI；若 15 秒内 AI 未恢复标签工具活动，才执行会话迁移兜底
      var baseSigs = (window.__glmDoneSigs || []).length;
      setTimeout(function () {
        try {
          var nowSigs = (window.__glmDoneSigs || []).length;
          if (nowSigs > baseSigs || window.__glmStreaming) { console.log('[GlmBridge] AI resumed, relay cancelled'); return; }
        } catch (e) {}
        if (!window.__glmRelayFired) glmStartRelay();
      }, 15000);
      return true;
    }
    return false;
  ...
  function extractStreamText(buf) {
    ...
    var full = '';
    var parts2 = String(buf).split(/\n/);
    for (var i = 0; i < parts2.length; i++) {
      var ln = parts2[i];
      if (/^data:\s*\{/.test(ln)) {
        try {
          var o = JSON.parse(ln.replace(/^data:\s*/, ''));
          if (o && o.status && o.status !== 'init') { try { window.__glmStreamStatus = o.status; } catch (e) {} }
          var pts = (o && o.parts) || [];
          var acc = [];
          for (var p = 0; p < pts.length; p++) {
            var pt = pts[p] || {};
            if (pt.role !== 'assistant') continue;
            var cs = pt.content || [];
            for (var c2 = 0; c2 < cs.length; c2++) {
              var it = cs[c2] || {};
              if (it.type === 'text' && typeof it.text === 'string' && it.text) acc.push(it.text);
              else if (it.type === 'code' && typeof it.code === 'string' && it.code) acc.push(it.code);
            }
          }
          full = acc.join('');
        } catch (e) {}
    ...
    var prev = window.__glmLastFull || '';
    window.__glmLastFull = full;
    if (full.length > prev.length && full.indexOf(prev) === 0) return full.slice(prev.length);
    if (full.length > prev.length) return full.slice(Math.max(0, prev.length - 40));
    return '';
  }
  var __glmParsedUpTo = 0;
  ...
  function handleStreamText(buf) {
    ...
    try {
      if (buf.indexOf('tool_call_id') >= 0 && (buf.indexOf('"tool"') >= 0 || buf.indexOf('block.tool') >= 0)) {
        window.__glmDiag.nativeToolBlocks = (window.__glmDiag.nativeToolBlocks || 0) + 1;
        ...
        var __tid = __tidM[1] || '';
        window.__glmNativeCalls = window.__glmNativeCalls || {};
        if (__tid && window.__glmNativeCalls[__tid]) { /* already handled */ }
        else {
          if (__tid) window.__glmNativeCalls[__tid] = 1;
          window.__glmDiag.nativeToolLast = __tbName + '|' + __taS.slice(0, 200);
          var __short = String(__tbName).replace(/^.*\.MT_MCP\./, '').replace(/^.*\.mt_/, 'mt_');
          if (__short && __short.indexOf('mt_') === 0 && !window.__glmBusy && !window.__glmPendingResult) {
            window.__glmDiag.nativeExecuted = (window.__glmDiag.nativeExecuted || 0) + 1;
            try { toolAndContinue(__short, __argsObj); }
            catch (eTb) { console.log('[GlmBridge] native exec err', String(eTb && eTb.message || eTb)); }
          }
        }
      }
    } catch (eNat2) {}
    ...
    if (!calls.length) {
      ...
      try {
        ...
        if (/\u8fd0\u884c\s*(python|\u4ee3\u7801)|```python|ipython|subprocess|\bexec\s*\(/i.test(_ct9)) {
          var _n9 = Date.now();
          if (!window.__glmPyWarnTs || _n9 - window.__glmPyWarnTs > 30000) {
            window.__glmPyWarn = (window.__glmPyWarn || 0) + 1;
            window.__glmPyWarnTs = _n9;
            if (window.__glmPyWarn <= 3) {
              glmSendGuide('[系统提示:禁止原生工具] 检测到你在输出 Python/代码块或代码执行意图——这些工具在沙箱中无法访问 Android 文件！必须用 <median_name> 标签调用本地工具继续任务。');
              console.log('[GlmBridge] python-code warn sent #' + window.__glmPyWarn);
            }
          }
        }
      } catch (ePy9) {}
    ...
    for (var i = 0; i < calls.length; i++) {
      ...
      if (isExampleCall(c)) { console.log('[GlmBridge] skip example call', fullN, JSON.stringify(c.args||{}).slice(0,80)); continue; }
      var sig = fullN + '|' + JSON.stringify(c.args || {});
      if (glmIsDupSig(sig)) continue;
  ...
  function glmInstallFetch() {
    ...
    if (isChat) { try { window.__glmDiag.reqs++; } catch (e) {} }
    ...
    function headersToObj(h) {
      ...
      try {
        ...
        if (_cur && _cur !== window.__glmLastPage) {
          window.__glmLastPage = _cur;
          window.__glmInjCount = 0;
        }
      } catch (e) {}
      try { if (bodyObj.conversation_id) window.__glmConvId = bodyObj.conversation_id; } catch (e) {}
      var found = false;
      for (var i = 0; i < msgs.length; i++) {
        ...
        for (var j = 0; j < cts.length; j++) {
          var it = cts[j] || {};
          if (it.type === 'text' && typeof it.text === 'string') { orig = it.text; break; }
        }
        if (orig === '') continue;
        found = true;
        if (orig.indexOf('[System: 工具执行结果]') < 0 && orig.indexOf('[System: You have access') < 0) {
          if (orig.trim() !== '') { try { glmResetDup(); } catch (e) {} window.__glmFailCount = 0; }
          var inj = '';
          if (window.__glmPendingResult) {
            inj = '[System: 工具执行结果]\n' + window.__glmPendingResult + '\n[结果结束。继续任务。]\n';
            window.__glmPendingResult = null;
          }
      ...
      if (!found) { try { window.__glmDiag.capReasons.push('no-user-msg'); } catch (e) {} return false; }
      return true;
    }

    if (isChat && init) {
      var capOk = false;
      if (!init.body) { window.__glmDiag.capReasons.push('no-body'); }
      else {
        try {
          ...
          if (typeof init.body === 'string') {
            ...
            window.__glmDiag.capReasons.push('body-type:' + Object.prototype.toString.call(init.body));
          }
          if (bodyObj && injectBlocks(bodyObj)) {
            ...
            window.__glmLastReqTs = Date.now();
            capOk = true;
          }
        } catch (e) {
          window.__glmDiag.capReasons.push('err:' + String(e && e.message || e));
        }
      }
      if (!capOk) console.log('[GlmBridge] capture skip', url.slice(-40), window.__glmDiag.capReasons.slice(-2));
    }
    ...
    if (isChat) {
      ...
      p.then(function (resp) {
        ...
        function finalize() {
          ...
          setTimeout(function () { try { if (window.__glmPendingResult && !window.__glmBusy) autoContinue(); } catch (eAc) {} }, 200);
          // v1.14.6: 检测到并发冲突错误时, 5s后重试挂起结果回传
          try {
            if (buf.indexOf('REASON_CHAT_IN_PROGRESS') >= 0 || buf.indexOf('CHAT_IN_PROGRESS') >= 0) {
              window.__glmDiag.chatInProgress = (window.__glmDiag.chatInProgress||0)+1;
              setTimeout(function () { try { if (window.__glmPendingResult && !window.__glmBusy && !window.__glmStreaming) autoContinue(); } catch (eCp) {} }, 5000);
            }
          } catch (eCp2) {}
          ...
          try {
            if (buf.indexOf('unauthenticated') >= 0 && buf.indexOf('token is expired') >= 0) {
              window.__glmDiag.tokenExpired = (window.__glmDiag.tokenExpired||0)+1;
              console.log('[GlmBridge] token expired detected, auto-resume in 6s');
              setTimeout(function () {
                try {
                  var _tok2 = localStorage.getItem('access_token');
                  if (_tok2 && !window.__glmStreaming && !window.__glmBusy) {
                    if (window.__glmLastReq) {
          ...
          try { var _cap = buf.length > 65536 ? buf.slice(buf.length - 65536) : buf; window.__glmLastResp = _cap; window.__glmLiveResp = _cap; } catch (e) {}
          // v1.9.2：完整流含 assistant 事件 → 服务端已回复，清除 ack 等待
          if (buf.indexOf('"role":"assistant"') >= 0) {
            window.__glmAckPending = false;
            window.__glmLastAssistTs = Date.now();
          }
          ...
          try {
            if (buf.indexOf('REASON_TOKEN_LENGTH_TOO_LONG') >= 0) {
              window.__glmDiag.tokenTooLong = (window.__glmDiag.tokenTooLong || 0) + 1;
              console.log('[GlmBridge] server TOKEN_LENGTH_TOO_LONG, auto-relay to new chat');
              var baseSigs2 = (window.__glmDoneSigs || []).length;
              setTimeout(function () {
                try {
                  var nowSigs2 = (window.__glmDoneSigs || []).length;
                  if (nowSigs2 > baseSigs2 || window.__glmStreaming) { console.log('[GlmBridge] AI resumed, relay cancelled'); return; }
                } catch (e) {}
                if (!window.__glmRelayFired) glmStartRelay('token-limit');
              }, 15000);
            }
          } catch (e3) {}
          var __dsAfter = (window.__glmDoneSigs || []).length;
          ...
          try {
            ...
            if (__dsAfter === __dsBefore && /budget exhausted|预算已耗尽|工具调用预算|25步.{0,12}(限制|预算)|Do not attempt to search|You have exhausted|超出限制|无法再调用任何工具/i.test(buf)) {
              var tnow2 = Date.now();
              if (!window.__glmToolBlockTs || tnow2 - window.__glmToolBlockTs > 120000) {
                window.__glmToolBlockTs = tnow2;
                ...
                setTimeout(function () {
                  if (!window.__glmStreaming && !window.__glmPendingResult) { try { glmSendGuide(guide2); } catch (e2) {} }
                }, 1200);
              }
            }
          } catch (e3) {}
          setTimeout(function () { if (window.__glmPendingResult && !window.__glmBusy) autoContinue(); }, 300);
        }
        try {
          var cl = resp.clone();
          var reader = cl.body && cl.body.getReader();
          if (!reader) { window.__glmStreaming = false; return; }
          function pump() {
            return reader.read().then(function (r) {
              ...
              window.__glmLiveResp = buf.length > 65536 ? buf.slice(buf.length - 65536) : buf;
              ...
              // 否则 __glmStreaming 卡死导致 autoContinue 永远不触发
              console.log('[GlmBridge] stream aborted, recover:', String(e && e.message || e));
          ...
          console.log('[GlmBridge] resp handler err:', String(e && e.message || e));
          window.__glmStreaming = false;
        }
      }, function () { window.__glmStreaming = false; });
    }
    return p;
  };
    try { window.__glmFetchWrapper = window.fetch; } catch (e) {}
  }
  glmInstallFetch();
  // v1.14.0: fetch 守护——被框架覆盖立即重装，保证消息请求始终经过教学注入
  setInterval(function () {
    try {
      if (!window.__glmFetchWrapper || window.fetch !== window.__glmFetchWrapper) glmInstallFetch();
    } catch (e) {}
  }, 2000);
  // v1.9.1 流看门狗（v1.14.3增强：挂起自动唤醒）——streaming 超过90s且无新数据时，
  // 复位并自动续跑；无 pendingResult 时协议层发「继续」唤醒 AI（服务端流静默中断场景）
  var __glmLastStreamLen = 0;
  setInterval(function () {
    try {
      if (window.__glmStreaming && window.__glmStreamTs) {
        var _curLen = (window.__glmLiveResp || '').length;
        if (Date.now() - window.__glmStreamTs > 90000 && _curLen === __glmLastStreamLen) {
          console.log('[GlmBridge] stream watchdog: force reset' + (window.__glmPendingResult ? ' (pending)' : ' (stall, wake up)'));
          window.__glmStreaming = false;
          if (window.__glmPendingResult && !window.__glmBusy) autoContinue();
          else if (!window.__glmBusy) { try { glmSendGuide('继续任务'); } catch (e2) {} }
        }
        __glmLastStreamLen = _curLen;
      } else {
        __glmLastStreamLen = 0;
      }
    } catch (e) {}
  }, 15000);
  ...
  setInterval(function () {
    try {
      if (!window.__glmAckPending) return;
      if (window.__glmStreaming) return;
      if (Date.now() - (window.__glmAckTs || 0) > 45000) {
        window.__glmAckPending = false;
        if (!window.__glmBusy) {
          console.log('[GlmBridge] ack timeout, UI fallback continue');
          if (uiSendText('继续')) window.__glmDiag.uiSends++;
        }
      }
    } catch (e) {}
  }, 10000);
  ...
  window.__glmPanelOpen = function (url) {
    try {
      var clean = cleanPanelUrl(url);
      window.__glmPanelCount = (window.__glmPanelCount || 0) + 1;
      console.log('[GlmBridge] panel open', String(clean).slice(0, 120));
      var x = new XMLHttpRequest();
      x.open('POST', window.__glmMcuBase(), true);
      x.timeout = 5000;
      x.setRequestHeader('Content-Type', 'application/json');
  ...
  (function () {
    ...
    document.addEventListener('click', function (ev) {
      try {
        ...
        window.__glmPanelOpen(abs);
      } catch (e) {}
    }, true);
  })();

  // 注：MCP 探测（同步 XHR）延迟执行，避免拖慢 document-start 计时窗口导致脚本被隔离
  setTimeout(function () { try { window.__glmMcuBase(); } catch (e) {} }, 1500);
  console.log('[GlmBridge] injected at document-start');
})();